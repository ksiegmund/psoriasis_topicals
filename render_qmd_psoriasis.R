...
d <- read.csv("data/psoriasis_topicals_data.csv")
...
# Save new .RData file with my shared_data_path in it
shared_data_path <- file.path(getwd(), "psoriasis_params.RData")
save.image(file = shared_data_path)

# Set a local, static file destination 
#local_qmd_file <- "runtime_adhd_management.qmd"
# I don't need to download this, I cloned the whole directory
#local_qmd_file <- "runtime_adhd_management.qmd"
#download.file(url = qmd_url, destfile = local_qmd_file, mode = "wb")
# needed to edit the original file source("") calls
local_qmd_file <- "runtime_psoriasis_management.qmd"

# Set up output file name strings
output_file <- glue::glue("psoriasis_management_{subgroup}.html")
...
output_file <- glue::glue("my_adhd_management_RCT_only_{subgroup}.html")

if (comparator_vs_int) {
  output_file <- 
    glue::glue("my_adhd_management_RCT_only_comparator_{subgroup}.html")
...
output_file <- glue::glue("my_adhd_management_no_high_RoB_{subgroup}.html")

if (comparator_vs_int) {
  output_file <- 
    glue::glue("my_adhd_management_no_high_RoB_comparator_{subgroup}.html")
}
...
if (comparator_vs_int) {
  output_file <- 
    glue::glue(
      "my_adhd_management_comp_stimulant_vs_int_nonstimulant_comparator_{subgroup}.html"
      )
}
